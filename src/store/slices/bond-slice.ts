...
export const calcBondDetails = createAsyncThunk(
  ...
  async ({ bondKey, value, provider, networkID, userBalance }: CalcBondDetailsPayload): Promise<BondDetails> => {
    ...
    const bondCalcContract = new ethers.Contract(addresses.NJORD_BONDING_CALC_ADDRESS, BondingCalcContract, provider);
    ...
    const rawMarketPrice = (await getMarketPrice(networkID, provider)).mul(bnbPrice);
