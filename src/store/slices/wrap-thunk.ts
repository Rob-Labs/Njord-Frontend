...
export const approveWrapping = createAsyncThunk('wrap/approve', async ({ provider, address, networkID }: ApproveWrappingProps, { dispatch }) => {
  if (!provider) {
...
export const changeWrap = createAsyncThunk('wrap/changeWrap', async ({ action, value, provider, address, networkID }: ChangeWrapProps, { dispatch }) => {
  if (!provider) {
    alert('Please connect your wallet!');
    return;
  }
