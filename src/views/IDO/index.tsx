...
import { IDOContract, BNBContract, StakingContract } from 'src/abi';
...
interface State {
  ...
  connected: boolean;
  finalized: boolean;
  walletBNBBalance?: string;
  walletBNBAllowance?: string;
  whitelisted: boolean;
  allotment?: string;
  idoBNBAmount?: string;
  ...
  switch (action.type) {
    ...
    case 'approved': {
      return {
        ...state,
        txPending: false,
        walletBNBAllowance: action.walletBNBAllowance,
      };
    }
    case 'purchasing': {
      return { ...state, txPending: true, error: undefined };
...
export default function IDO() {
  ...
  const mai = new ethers.Contract(addresses.BNB_ADDRESS, BNBContract, provider);
  ...
  const loadDetails = useCallback(async () => {
    const idoBNBAmount = ethers.utils.formatEther(await mai.balanceOf(ido.address));
    let walletBNBBalance = connected ? await mai.balanceOf(wallet) : BigNumber.from(0);
    walletBNBBalance = walletBNBBalance.lt('1000000000') ? '0' : ethers.utils.formatEther(walletBNBBalance);
    const walletBNBAllowance = connected ? ethers.utils.formatEther(await mai.allowance(wallet, ido.address)) : '0';
  ...
  const NotConnectedBox = () => (
    <div className={styles.notConnectedBox}>
      <p className={styles.title}>
        Claim your <span className={styles.highlight}>NJORD</span> to join the Otter Kingdom now!
      </p>
  ...
  const SuccessBox = () => (
    <>
      <div className={styles.presale_header_success}>
        <h1 className={styles.successTitle}>You’ve claimed your NJORD!</h1>
          ...
          <div className={styles.balance_stats}>
            ...
            <div className={styles.tokenAmounts}>
              <p className={styles.tokenAmount}>
                ...
                <span className={styles.tokenTitle}>NJORD</span>
              </p>
            </div>
          </div>
        )}
          ...
          <div className={styles.contribution}>
            ...
            <div className={styles.tokenAmounts}>
              <p className={styles.tokenAmount}>
                ...
                <span className={styles.tokenTitle}>BNB</span>
              </p>
              <p className={styles.tokenSubTitle}>5 BNB = 1 NJORD</p>
            </div>
          </div>
        )}
        {state.purchasedAmount != '0.0' && (
